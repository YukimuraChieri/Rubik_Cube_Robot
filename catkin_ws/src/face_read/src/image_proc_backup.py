...
'''
# 红色颜色分量范围
RedLower = np.array([0, 43, 43])
RedUpper = np.array([8, 255, 255])
# 绿色颜色分量范围
GreenLower = np.array([35, 43, 43])
GreenUpper = np.array([89, 255, 255])
# 黄色颜色分量范围
YellowLower = np.array([20, 43, 43])
YellowUpper = np.array([32, 255, 255])
# 蓝色颜色分量范围
BlueLower = np.array([90, 43, 43])
BlueUpper = np.array([120, 255, 255])
# 橙色颜色分量范围
OrangeLower = np.array([9, 43, 43])
OrangeUpper = np.array([24, 255, 255])
# 白色颜色分量范围
WhiteLower = np.array([0, 0, 128])
...
class FaceBoxs(object):
    ...
    cube_box_size = [50, 50]    # 取色框大小
    box_origin = [25, 25]       # 取色框原点坐标
...
class CubeReadFace(object):
    ...
    # 单面读取函数
    def readOneFace(self, para_boxs, img, gain):
        ...
        for i, box in enumerate(box_list):
            box = self.blueLoss(box, gain)  # 取色框图像蓝光抑制
            hsv = self.HSV_mean(box)        # 求取色框图像HSV均值
            # print(hsv)
            mask_U = cv2.inRange(hsv, YellowLower, YellowUpper)  # 识别黄色
            mask_R = cv2.inRange(hsv, RedLower, RedUpper)  # 识别红色
    ...
    def readTwoFaces(self):
        up_face = self.readOneFace(para_boxs=self.up_boxs, img=self.img, gain=1.0)        # 读取上表面色块序列
        down_face = self.readOneFace(para_boxs=self.down_boxs, img=self.img, gain=1.0)    # 读取下表面色块序列
...
if __name__ == "__main__":
    ...
    left_faces = CubeReadFace(up_boxs=up_boxs, down_boxs=down_boxs, img=left_img)
    ...
    right_faces = CubeReadFace(up_boxs=up_boxs, down_boxs=down_boxs, img=right_img)
